...
import { initialState as profileInitialState } from 'features/contractor/profile/reducer';
...
import { initialState as contractorOrganizationInitialState } from 'features/client/contractorOrganization/reducer';
...
const { organizations, userInfo, contactInformation, error } = fixtures;
const contractor = organizations[2];
const client = organizations[3];
...
const state = {
  organization: { ...organizationInitialState, organization: client },
  contractorOrganization: { ...contractorOrganizationInitialState, organization: contractor },
...
beforeEach(() => {
  axiosMock
    .onGet(`/api/v3.01/organizations(${contractor.id})/contactInformation`)
    .reply(200, { value: contactInformation });

  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/images`).reply(200, response);

  axiosMock
    .onGet(`/files/v3.01/organizations(${contractor.id})/imagethumbnails(${uploadedImage.id})`)
    .reply(200, new Blob(['foobar'], { type: 'image/jpeg' }), { 'content-type': 'image/jpeg' });

  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/logo`).reply(200, logoMetaData);

  axiosMock
    .onGet(`/files/v3.01/organizations(${contractor.id})/logo`)
    .reply(200, new Blob(['foobar'], { type: 'image/jpeg' }), { 'content-type': 'image/jpeg' });

  axiosMock.onPost('/spapi/errors').reply(200);
});

describe('client jumbotron container', () => {
  it('renders a loading spinner right away', () => {
    const wrapper = mount(
      <FakeStoreAndRouter<Partial<RootState>>
        state={{ ...state, contractorOrganization: { ...contractorOrganizationInitialState, isFetching: true } }}
        initialEntries={[`/organization/${client.id}/connect/profile/${contractor.id}`]}
      >
        <Route path="/organization/:organizationId/connect/profile/:contractorId" component={JumbotronContainer} />
      </FakeStoreAndRouter>
    );
  ...
  it('renders an error message if there was an error fetching', () => {
    const wrapper = mount(
      <FakeStoreAndRouter<Partial<RootState>>
        state={{ ...state, contractorOrganization: { ...contractorOrganizationInitialState, error } }}
        initialEntries={[`/organization/${contractor.id}/connect/profile/${contractor.id}`]}
      >
        <Route path="/organization/:organizationId/connect/profile/:contractorId" component={JumbotronContainer} />
      </FakeStoreAndRouter>
    );
  ...
  it('renders the company name on the header', () => {
    ...
    expect(wrapper.find('h4').text()).toEqual(contractor.name);
  });
});
