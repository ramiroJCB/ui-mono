...
beforeEach(() => {
  ...
  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/projects`).reply(200, projects);
  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/licenses`).reply(200, licenses);
  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/accreditations`).reply(200, accreditations);
  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/certifications`).reply(200, certifications);
  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/references`).reply(200, references);
  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/officeLocations`).reply(200, officeLocations);
  axiosMock.onGet(`/api/v3.01/organizations(${contractor.id})/tradeNames`).reply(200, tradeNames);
  axiosMock.onPost('/spapi/errors').reply(200);
});

describe('client profile content container', () => {
  it('renders a loading spinner right away', () => {
    const wrapper = mount(
      ...
      >
        <Route path="/organization/:organizationId/connect/profile/:contractorId" component={ProfileContentContainer} />
      </FakeStoreAndRouter>
    );
