...
import {
  Actions as FetchPhotoGalleryThumbnailsActions,
  fetchPhotoGalleryThumbnails,
  fetchPhotoGalleryThumbnailsFailure,
  fetchPhotoGalleryThumbnailsRequest,
  fetchPhotoGalleryThumbnailsSuccess
} from './fetchPhotoGalleryThumbnails';
import {
...
type Actions = FetchPhotoGalleryThumbnailsActions | FetchPhotoGalleryMetaDataActions;
...
describe('fetch photo gallery thumbnails', () => {
  it('dipatches a success action when fetching uploaded thumbnails', async () => {
    axiosMock
    ...
    await store.dispatch(fetchPhotoGalleryThumbnails(organizationId));

    const actions = store.getActions();

    expect(actions[0]).toEqual(fetchPhotoGalleryThumbnailsRequest());
    ...
    expect(actions[3]).toEqual(fetchPhotoGalleryThumbnailsSuccess(acceptedImages));
  });

  it('dispatches a failure action due to a network error', async () => {
    try {
      axiosMock.onPost('/spapi/errors').reply(200);
      axiosMock.onGet(`/files/v3.01/organizations(${organizationId})/imagethumbnails(${metaData.id})`).networkError();
      ...
      expect(actions[1]).toEqual(fetchPhotoGalleryMetaDataRequest());
      expect(actions[2]).toEqual(fetchPhotoGalleryMetaDataSuccess(value, organizationId));
      expect(actions[3]).toEqual(fetchPhotoGalleryThumbnailsFailure(error));
    }
  });
});
