...
import { fetchOperationalContractorPeriodsIfNeeded } from '../actions/fetchOperationalContractorPeriods';
...
const mapStateToProps = (state: RootState, { periodId }: OwnProps) => {
  const { isFetching, error, periods } = state.operationalContractorPeriods;
...
class ContractorPeriods extends React.Component<Props> {
  ...
  handleMenuItemClick = (periodId: string) => () => {
    ...
    history.push(`/${organizationId}/reporting/operations/clients/${clientId}/periods/${periodId}`);
  };
