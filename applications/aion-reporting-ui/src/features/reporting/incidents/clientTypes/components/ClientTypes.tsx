...
const ClientTypes: React.FC<Props> = ({
  incidentTypes,
  classes,
  clientId,
  showInactive,
    ...
    <React.Fragment>
      <GridContainer alignItems="flex-start" justify="space-between" style={{ flex: 0 }}>
        <Grid item>
          ...
          >
            {t('reporting.incidents.clientTypes.indicatorBehaviorCategories', 'Indicator Behavior Categories')}
          </BackTitleHeader>
        </Grid>
        <Grid item>
          <FormControlLabel
            ...
            label={t(
              'reporting.incidents.clientTypes.showInactiveBehaviorCategories',
              'Show Inactive Behavior Categories'
            )}
          />
        </Grid>
      </GridContainer>
              ...
              {incidentTypeId && (
                <Grid item>
                  <Button fullWidth variant="contained" color="secondary" onClick={handleAddClick}>
                    {t('reporting.common.addBehaviorCategory', 'Add a Behavior Category')}
                  </Button>
                </Grid>
              )}
            </GridContainer>
            <GridContainer>
              {!isFetching && incidentTypes ? (
                incidentTypes.length > 0 ? (
                  incidentTypes.map(({ id, name, description }) => (
                    <Grid item xs={12} key={id}>
                      <Typography variant="body1">
                        <Link to={`/${clientId}/reporting/incidents/types/edit/${id}`}>{name}</Link>
                      </Typography>
                  ...
                  <Grid item xs={12}>
                    <Typography variant="subtitle1" className={classes.noneFound}>
                      {t(
                        'reporting.incidents.clientTypes.noBehaviorCategoriesAdded',
                        'No behavior categories have been added.'
                      )}
                    </Typography>
                  </Grid>
                )
...
export const ClientTypesComponent = withStyles(styles)(ClientTypes);
