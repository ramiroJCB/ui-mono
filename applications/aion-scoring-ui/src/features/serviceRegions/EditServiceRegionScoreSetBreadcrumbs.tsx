...
export const EditServiceRegionScoreSetBreadcrumbs: React.FC = () => {
  ...
  if (matchPath(pathname, '/:organizationId/scoring/service-regions/:serviceRegionId/score-set/:scoreSetId')) {
    links.push({
      to: {
  ...
  if (
    matchPath(
      pathname,
      '/:organizationId/scoring/service-regions/:serviceRegionId/score-set/:scoreSetId/score-item/add'
    )
  ) {
    links.push({
      to: {
        pathname: `/${organizationId}/scoring/service-regions/${serviceRegionId}/score-set/${scoreSetId}/score-item/add`,
        search
      },
      label: !isFetchingScoreSet && scoreSet ? `Add Scoring Item` : '⋯'
    });
  }
