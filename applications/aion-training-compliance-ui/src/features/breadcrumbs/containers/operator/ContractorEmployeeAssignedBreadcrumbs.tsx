...
class ContractorEmployeeAssignedBreadcrumbs extends React.Component<Props> {
  ...
  render() {
    ...
    if (employeeId && !isFetchingEmployee && employee) {
      links.push({
        to: {
          pathname: `/${organizationId}/training-compliance/contractors/${contractorId}/assigned-employees/${employeeId}`
        },
        label: employee.name
      });
    }
    ...
    if (employeeTrainingRequirementId && !isFetchingEmployeeTrainingRequirement && employeeTrainingRequirement) {
      links.push({
        to: {
          pathname: `/${organizationId}/training-compliance/contractors/${contractorId}/assigned-employees/${employeeId}/training/${employeeTrainingRequirementId}`
        },
...
export const ContractorEmployeeAssignedBreadcrumbsContainer = withRouter(
  connect(mapStateToProps, mapDispatchToProps)(withTranslation()(ContractorEmployeeAssignedBreadcrumbs))
);
