...
import { ViewClientInspections } from '../ViewInspections';

const axiosMock = new MockAdapter(axios);

describe('view client inspections', () => {
  beforeEach(() => {
    axiosMock.reset();
    axiosMock
  ...
  describe('screens < 600px wide', () => {
    it('shows a loading spinner, followed by a virtualized grid on smaller screens', async () => {
      ...
      const { store } = renderWithProviders(<ViewClientInspections />, {
  ...
  describe('screens > 1280px wide', () => {
    it('shows a loading spinner, followed by a table of inspections', async () => {
      ...
      const { store } = renderWithProviders(<ViewClientInspections />, { initialState: { inspections: initialState } });
